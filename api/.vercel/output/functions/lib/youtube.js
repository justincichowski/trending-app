...
async function getYouTubeVideos(params) {
    ...
    if (Date.now() < YT_CIRCUIT_TRIPPED_UNTIL) {
        const key = (0, persist_1.hashKey)({ params, _circuit: true });
        const cached = (0, persist_1.getCache)(key);
        if (cached)
            return cached;
        (0, persist_1.setCache)(key, [], YT_CACHE_EMPTY_TTL_MS);
        return [];
    }
    // Cache key & in-flight de-duplication
    const key = (0, persist_1.hashKey)({ params });
    const cached = (0, persist_1.getCache)(key);
    if (cached)
        return cached;
    const inflight = (0, persist_1.getInflight)(key);
    if (inflight)
        return inflight;
    const _promise = (async () => {
        ...
        try {
            if (playlistId) {
                ...
                (0, persist_1.setCache)(key, normalized, normalized.length ? YT_CACHE_TTL_MS : YT_CACHE_EMPTY_TTL_MS);
                return normalized;
            }
            if (query) {
    ...
    (0, persist_1.setInflight)(key, _promise);
    return _promise;
}
