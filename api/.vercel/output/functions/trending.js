...
const RIGHT_PANEL_TTL_MS = 15 * 60 * 1000; // 15 minutes
...
async function fetchAll() {
    const results = await Promise.allSettled(TRENDING_FEEDS.map(feed => (0, rss_1.getRssFeed)({ url: feed.url, source: feed.source })));
    const data = {};
...
async function handler(req, res) {
    ...
    try {
        ...
        const data = await (0, persist_1.cached)('trending_v2', RIGHT_PANEL_TTL_MS, async () => {
            ...
            if (debug)
                console.log('[API /trending] first fetch keys:', Object.keys(d1));
            if (isMeaningful(d1))
                return d1;
            // retry once after 1s
            if (debug)
            ...
            if (debug)
                console.log('[API /trending] second fetch keys:', Object.keys(d2));
            return d2;
        });
        if (debug)
            console.log('[API /trending] post-cache meaningful=', isMeaningful(data));
        if (!isMeaningful(data)) {
            if (debug)
                console.log('[API /trending] returning 204 No Content');
            return res.status(204).end();
        }
        console.log('[API /trending] returning sections:', Object.keys(data));
        if (debug)
            console.log('[API /trending] sample section lengths:', Object.fromEntries(Object.entries(data).map(([k, v]) => [k, v.length])));
