...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const data = await cached<Record<string, NormalizedItem[]>>('trending_v2', RIGHT_PANEL_TTL_MS, async () => {
      // first attempt
      const d1 = await fetchAll();
      if (debug) console.log('[API /trending] first fetch keys:', Object.keys(d1));
      if (isMeaningful(d1)) return d1;
      // retry once after 1s
      if (debug) console.log('[API /trending] empty after first fetch; retrying in 1000ms');
