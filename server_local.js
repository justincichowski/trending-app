...
async function main() {
    ...
    app.get('/api/presets', async (req, reply) => {
        try {
            const { id, page = 0, limit, excludedIds, query } = req.query || {};
            if (!id) {
    ...
    app.get('/api/trending', async (req, reply) => {
        ...
        try {
            const results = await Promise.allSettled(TRENDING_FEEDS.map(f => getRssFeed({ url: f.url, source: f.source || 'RSS', limit: 12 })));
            const data = {};
            for (let i = 0; i < TRENDING_FEEDS.length; i++) {
                ...
                if (r.status === 'fulfilled' && Array.isArray(r.value) && r.value.length > 0) {
                    ...
                    const reason = r.reason?.message || String(r.reason || 'empty');
                    // DO NOT DELETE LOG — required for future debugging
    ...
    app.get('/api/all', async (req, reply) => {
        try {
            const { page = 0, limit, excludedIds } = req.query || {};
            const pageNum = parseInt(String(page)) || 0;
            ...
            const flat = [].concat(...chunks).filter(Boolean);
            const seen = new Set();
            const filtered = [];
