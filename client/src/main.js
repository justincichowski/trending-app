...
async function fetchTrendingOnce() {
    const base = import.meta.env.DEV ? 'http://localhost:3000/api' : (window.VITE_API_URL || import.meta.env.VITE_API_URL || '/api');
 ...
 */
export function playYouTubeVideo(imageContainer, itemId) {
    ...
    currentPlayer = new window.YT.Player(playerId, {
        height: '100%',
        width: '100%',
        videoId: videoId,
        playerVars: {
 ...
 */
export async function categoryView(params) {
        ...
        if (logoWrapper) {
            logoWrapper.addEventListener('mouseover', () => {
                ...
                if (lastUpdated && (currentPath === '/' || currentCategory?.id === 'favorites')) {
                    tooltip.show(logoWrapper, `Last updated: ${new Date(lastUpdated).toLocaleTimeString()}`);
                }
            });
...
if (searchInput) {
    ...
    autocomplete = new Autocomplete(searchInput, (selectedValue) => {
...
document.addEventListener('contextmenu', (event) => {
    ...
    if (target.classList.contains('item-image')) {
        ...
        const itemCard = target.closest('.item-card');
        const titleLink = itemCard?.querySelector('.item-title a');
